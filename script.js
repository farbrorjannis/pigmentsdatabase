...
async function loadPigments() {
    try {
        ...
        document.getElementById('results').innerHTML = 
            '<p class="hint">❌ Could not load the pigment database.</p>';
    }
}
...
function searchPigments() {
    ...
    if (searchTerm === '') {
        document.getElementById('results').innerHTML = '<p class="hint">⬆️ Type a search term above ⬆️</p>';
        return;
    }
...
function displayResults(pigments) {
    ...
    if (pigments.length === 0) {
        resultsContainer.innerHTML = '<p class="hint">🔍 No pigments found.</p>';
        return;
    }
    let html = '';
    pigments.forEach(p => {
